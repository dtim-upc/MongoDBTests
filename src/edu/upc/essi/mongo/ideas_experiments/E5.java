...
public class E5 {
	...
	public static void generate(CSVWriter writer) throws Exception {
		...
		for (int attribs : Lists.newArrayList(1, 2, 4, 8, 16, 32, 64, 128)) {
			...
			JsonObject templateWithLength = generateTemplate(attribs, false, fixedattibs);
			...
			for (int i = 0; i < 10; ++i) {
				gen.generateFromPseudoJSONSchema(10, fileForTemplateWithCount.getAbsolutePath()).stream()
...
//
			...
			for (int i = 0; i < 10; ++i) {
				gen.generateFromPseudoJSONSchema(10, fileForTemplateWithLength.getAbsolutePath()).stream()
						.map(d -> Document.parse(d.toString())).forEach(DocumentSet.getInstance().documents::add);
	...
	private static JsonObject generateTemplate(int attribs, boolean addSiblings, int attributes) {
		...
		if (addSiblings) {
			...
			for (int i = 1; i < attributes + 1; i++) {
				...
				if (i == attributes) {
					properties.add("a" + sb.toString() + (i < 10 ? "00" + String.valueOf(i)
							: i < 100 ? '0' + String.valueOf(i) : String.valueOf(i)), A);
				} else {
					properties.add("a" + (i < 10 ? "00" + String.valueOf(i)
							: i < 100 ? '0' + String.valueOf(i) : String.valueOf(i)), A);
				}
			}
			out.add("properties", properties);
