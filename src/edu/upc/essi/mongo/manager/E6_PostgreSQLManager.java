...
public class E6_PostgreSQLManager {
	...
	public static E6_PostgreSQLManager getInstance(String table, int attributes, JsonObject JSONSchema, CSVWriter writer) throws Exception {
		if (instance == null)
			instance = new E6_PostgreSQLManager(table, attributes, JSONSchema, writer);
		return instance;
	}

	public E6_PostgreSQLManager(String table, int attributes, JsonObject JSONSchema, CSVWriter writer2) throws Exception {
		this.table = table;
...
//		JDBC = DriverManager.getConnection("jdbc:postgresql://10.55.0.32/ideas_experiments", "postgres", "user");
		...
		JDBC.createStatement().execute("CREATE EXTENSION \"postgres-json-schema\"");JDBC.commit();

		JDBC.createStatement().execute("CREATE TABLE " + table + "_JSON_withVal (ID CHAR(24), JSON JSONB)");
		JDBC.createStatement().execute("ALTER TABLE "+table+"_JSON_withVal ADD CONSTRAINT data_is_valid CHECK (validate_json_schema('"+JSONSchema.toString()+"', JSON))");
		JDBC.createStatement().execute("CREATE TABLE " + table + "_JSON_withoutVal (ID CHAR(24), JSON JSONB)");
		JDBC.createStatement().execute("CREATE TABLE " + table + "_TUPLE (ID CHAR(24),"+
				JSONSchema.getJsonArray("required").
						stream().map(t -> t.toString()+" int").collect(Collectors.joining(","))+")");
		JSONSchema.getJsonObject("properties").keySet().forEach(k -> {
			...
			try {
				JDBC.createStatement().execute("ALTER TABLE "+table + "_TUPLE ADD CONSTRAINT validate"+k+" CHECK ("+k+" BETWEEN "+min+" AND "+max+")");
	...
	public void insert(String kind) throws SQLException {
		...
		if (kind.contains("JSON")) {
			...
			DocumentSet.getInstance().documents.stream().map(d -> "INSERT INTO " + table + kind + " (ID,"+
					d.keySet().stream().filter(k->!k.equals("_id")).sorted().collect(Collectors.joining(","))
					+") VALUES ('" + d.get("_id") + "',"+
					d.keySet().stream().filter(k->!k.equals("_id")).sorted().map(k->String.valueOf(d.getInteger(k))).collect(Collectors.joining(","))
					+")").forEach(s -> {
				try {
					statement.addBatch(s);
				} catch (SQLException exc) {
		...
		writer.writeNext(new String[] { "Postgres", "insert", table + kind,
	...
	public void sum(String kind) throws Exception {
		...
		long elapsedTime = System.nanoTime() - startTime;
		writer.writeNext(new String[] { "Postgres", "sum", table + kind, String.valueOf(attributes),
	...
	public void size(String kind) throws SQLException {
		...
		ResultSet rs = stmt.executeQuery();
		rs.next();
		writer.writeNext(new String[] { "Postgres", "size", table+kind, String.valueOf(attributes),
				"", rs.getString(1) });
	}
