...
public class E5_PostgreSQLManager {
	...
	public static E5_PostgreSQLManager getInstance(String table, int nLevels, int nAttributes, CSVWriter writer)
			throws Exception {
		if (instance == null)
			instance = new E5_PostgreSQLManager(table, nLevels, nAttributes, writer);
		return instance;
	}

	public E5_PostgreSQLManager(String table, int nLevels, int nAttributes, CSVWriter writer2) throws Exception {
		this.table = table;
...
//		JDBC = DriverManager.getConnection("jdbc:postgresql://10.55.0.32/ideas_experiments", "postgres", "user");
	...
	public void insert() throws SQLException {
		...
		writer.writeNext(new String[] { "Postgres", "insert", table.substring(table.lastIndexOf("_") + 1), String.valueOf(attribs),
	...
	public void sum(boolean isLength) throws Exception {
		...
		path += (nAttributes < 10 ? "00" + String.valueOf(nAttributes)
				: nAttributes < 100 ? '0' + String.valueOf(nAttributes) : nAttributes) + "'" + ")::int) ";
		...
		long elapsedTime = System.nanoTime() - startTime;
		writer.writeNext(new String[] { "Postgres", "sum", table.substring(table.lastIndexOf("_") + 1), String.valueOf(attribs), String.valueOf(nAttributes),
	...
	public void size() throws SQLException {
		...
		ResultSet rs = stmt.executeQuery();
		rs.next();
		writer.writeNext(new String[] { "Postgres", "size", table.substring(table.lastIndexOf("_") + 1), String.valueOf(attribs), String.valueOf(nAttributes),
				"", rs.getString(1) });
	}
